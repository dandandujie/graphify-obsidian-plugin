...
> This directory contains the Python analysis engine used by the Obsidian plugin in the repository root.
> Primary product docs live at the root [README](../README.md).
>
> Obsidian-first stable contract:
> `graphify obsidian index|update|query|report|ingest|watch`

**Python analysis engine.** Build and update local knowledge graphs, then serve structured answers to the Obsidian plugin through stable CLI JSON responses.

> Transition note (v0.3.x): graphify now includes an **Obsidian-first** CLI contract for plugin integration:
> `graphify obsidian index|update|query|report|ingest|watch`.
> Legacy multi-platform skill install commands are still available for one transition release and are marked deprecated.
> Legacy standalone commands (`query`, `benchmark`, `hook`) are removed from the default CLI surface.
